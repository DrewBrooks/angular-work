...
export class ContentListComponent implements OnInit {
  constructor() {
    ...
    const contentPromise = new Promise((success, fail) => {
      const testPass = false;
      if (testPass) {
        //success(this.contentList.title);
      } else {
        fail('Failed');
      }
    });

    contentPromise.then((success: string) => {
      console.log('Success! ', success);
    }).catch((failure: string) => console.log('Failure: ', failure));
  }
  contentList: Content[];
